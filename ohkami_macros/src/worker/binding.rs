...
impl Binding {
    ...
    pub fn tokens_extract_from_env(&self, name: &Ident) -> TokenStream {
        ...
        let from_env = |getter: TokenStream| {
            quote! {
                #name: env.#getter?
            }
        };
...
impl Binding {
    pub fn collect_from_env(env_name: Option<Ident>) -> Result<Vec<(Ident, Self)>, syn::Error> {
        ...
        let config = match env_name.as_ref() {
            None => config,
            Some(name) => {
                let config = config.env.get_mut(&name.to_string()).ok_or_else(|| {
                    syn::Error::new(
                        name.span(),
                        format!("env `{name}` is not found in wrangler config"),
                    )
                })?;
                std::mem::take(config)
            }
        ...
        collection
            .into_iter()
            .map(|(name, binding)| {
                let name = syn::parse_str(&name).map_err(|_| {
                    syn::Error::new(
                        Span::call_site(),
