...
pub(crate) fn extract_doc_attrs(attrs: &[syn::Attribute]) -> Vec<syn::Attribute> {
    attrs
        ...
        .filter(|a| {
            a.meta
                .require_name_value()
                .is_ok_and(|m| m.path.get_ident().is_some_and(|i| i == "doc"))
        })
        .map(Attribute::clone)
        .collect()
}
...
pub(crate) fn find_file_at_package_or_workspace_root(
    ...
    match File::open(file_path) {
        ...
        Err(e) if matches!(e.kind(), ErrorKind::NotFound) => find_file_at_workspace_root(file_path),
        Err(e) => Err(e),
    }
}
