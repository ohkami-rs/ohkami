...
impl<'c> SetCookie<'c> {
    ...
    pub(crate) fn from_raw(str: &'c str) -> Result<Self, String> {
        ...
        let mut this = {
            let name = std::str::from_utf8(r.read_until(b"="))
                .map_err(|e| format!("Invalid Cookie name: {e}"))?;
            r.consume("=")
                .ok_or_else(|| format!("No `=` found in a `Set-Cookie` header value"))?;
        ...
        while r.consume("; ").is_some() {
            ...
            match r.consume_oneof([
                ...
                Some(0) => {
                    r.consume("=")
                        .ok_or_else(|| format!("Invalid `Expires`: No `=` found"))?;
                    let value = std::str::from_utf8(r.read_until(b"; "))
                        .map_err(|e| format!("Invalid `Expires`: {e}"))?;
                    this.expires = Some(Cow::Borrowed(value))
                }
                Some(1) => {
                    r.consume("=")
                        .ok_or_else(|| format!("Invalid `Max-Age`: No `=` found"))?;
                    let value = r
                        .read_until(b"; ")
                        .iter()
                        .fold(0, |secs, d| 10 * secs + (*d - b'0') as u64);
                    this.max_age = Some(value)
                }
                Some(2) => {
                    r.consume("=")
                        .ok_or_else(|| format!("Invalid `Domain`: No `=` found"))?;
                    let value = std::str::from_utf8(r.read_until(b"; "))
                ...
                Some(3) => {
                    r.consume("=")
                        .ok_or_else(|| format!("Invalid `Path`: No `=` found"))?;
                    let value = std::str::from_utf8(r.read_until(b"; "))
                ...
                Some(4) => {
                    r.consume("=")
                        .ok_or_else(|| format!("Invalid `SameSite`: No `=` found"))?;
...
impl SetCookieBuilder {
    ...
    pub(crate) fn new(
        ...
        cookie_value: impl Into<Cow<'static, str>>,
    ) -> Self {
        Self(SetCookie {
    ...
    pub(crate) fn build(self) -> String {
        ...
        unsafe {
            // SAFETY: All fields and punctuaters is UTF-8
