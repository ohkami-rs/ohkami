use crate::response::{err::ErrResponse, Response};
...
pub trait ElseResponse {
    type Expect;
    fn _else<F: FnOnce() -> ErrResponse>(self, error_response: F) -> Response<Self::Expect>;
}
impl<T> ElseResponse for Option<T> {
    type Expect = T;
    #[inline] fn _else<F: FnOnce() -> ErrResponse>(self, error_response: F) -> Response<Self::Expect> {
...
pub trait ElseResponseWithErr<E> {
    type Expect;
    fn _else<F: FnOnce(E) -> ErrResponse>(self, error_response: F) -> Response<Self::Expect>;
}
impl<T, E> ElseResponseWithErr<E> for std::result::Result<T, E> {
    type Expect = T;
    #[inline] fn _else<F: FnOnce(E) -> ErrResponse>(self, error_response: F) -> Response<Self::Expect> {
