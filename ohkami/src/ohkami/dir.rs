...
use crate::header::{AcceptEncoding, CompressionEncoding, ETag, Encoding};
use ohkami_lib::{map::TupleMap, time::ImfFixdate};
...
impl StaticFile {
    fn new(path: &Path) -> io::Result<(Self, std::borrow::Cow<'_, Path>)> {
        ...
        match CompressionEncoding::from_file_path(path) {
            None => Ok((Self::Source { file }, path.into())),
            Some((encoding, source)) => Ok((Self::Compressed { encoding, file }, source.into())),
        }
    }
...
impl Dir {
    pub(super) fn new(route: &'static str, dir_path: &Path) -> io::Result<Self> {
        ...
        {
            fn fetch_entries(dir: PathBuf) -> io::Result<Vec<PathBuf>> {
                dir.read_dir()?.map(|de| de.map(|de| de.path())).collect()
            }
        ...
        let (source_file, compressed_files) =
            {
                let source =
                    files
                        .swap_remove(files.iter().position(StaticFile::is_source).ok_or_else(
                            || {
                                io::Error::new(
                                    io::ErrorKind::InvalidData,
                ...
                let mut compressed = files
                    ...
                    .map(|f| {
                        ...
                        let StaticFile::Compressed { encoding, file } = f else {
                            unreachable!()
                        };
                        (encoding, file)
                    })
        ...
        Handler::new(
            |req| {
                Box::pin(async {
                    ...
                    if let (Some(if_none_match), Some(etag)) =
                        (req.headers.if_none_match(), &this.etag)
                    {
                    ...
                    let (encoding, content) = {
                        ...
                        crate::DEBUG!(
                            "[Dir] precompressed canidadates: {:?}",
                            this.compressed.iter().map(|(ce, _)| ce).collect::<Vec<_>>()
                        );
                    ...
                    Response::OK()
                        ...
                        .with_headers(|h| {
                            h.last_modified(&*this.last_modified_str)
                                .etag(this.etag.as_ref().map(ETag::serialize))
                                .content_encoding(
                                    encoding.map(CompressionEncoding::to_content_encoding),
                                )
                        })
                })
            },
            #[cfg(feature = "openapi")]
            {
                ...
                openapi::Operation::with(openapi::Responses::new([
                    ...
                    (304, openapi::Response::when("Not Modified")),
                ]))
            },
        )
    }
