...
use crate::{percent_decode, percent_decode_utf8};
use std::borrow::Cow;
        ...
        match &self.side {
            ParsingSide::Key => match next_punc {
                ...
                None => Err(serde::de::Error::custom(
                    "invalid key-value: unexpected end of input",
                )),
...
impl<'u, 'de> serde::Deserializer<'de> for &'u mut URLEncodedDeserializer<'de> {
        ...
        V: serde::de::Visitor<'de>,
    {
        ...
        match percent_decode_utf8(section).map_err(|e| {
            serde::de::Error::custom(format!(
                "Expected to be decoded to an UTF-8, but got `{}`: {e}",
        ...
        V: serde::de::Visitor<'de>,
    {
        ...
        let section = std::str::from_utf8(section).map_err(|_| {
            serde::de::Error::custom(format!(
    ...
    impl<'e, 'de> serde::de::VariantAccess<'de> for Enum<'e, 'de> {
            ...
            T: serde::de::DeserializeSeed<'de>,
        {
            ...
            Err(serde::de::Error::custom(
                "ohkami's builtin urlencoded deserializer doesn't support enum with newtype variants !",
            ))
        }
        ...
        {
            Err(serde::de::Error::custom(
                "ohkami's builtin urlencoded deserializer doesn't support enum with struct variants !",
            ))
        }
        ...
        {
            Err(serde::de::Error::custom(
                "ohkami's builtin urlencoded deserializer doesn't support enum with tuple variants !",
            ))
        }
    }
};
